...
function projectGeneratorLabel(task) {
  if (task?.project_generator === 'codex_cli' || task?.project_generation_provider === 'codex') {
    return `Codex CLI${task.project_generation_model ? `（${task.project_generation_model}）` : ''}`;
  }
...
function projectGeneratorProjectLabel(task) {
  if (task?.project_generator === 'codex_cli' || task?.project_generation_provider === 'codex') return 'Codex 项目';
  return task?.project_generation_provider === 'deepseek' ? 'DeepSeek 项目' : 'Claude 项目';
}

function projectGeneratorActor(task) {
  return projectGeneratorProjectLabel(task).replace(/ 项目$/, '');
}
        ...
        <div className="drawer-backdrop" onClick={() => setActiveTask(null)}>
          <aside className="detail-drawer" onClick={(event) => event.stopPropagation()}>
            ...
            <dl className="detail-list">
              ...
              <div><dt>生产流程</dt><dd>{activeTask.project_origin === 'generated_0to1' ? (Number(activeTask.workflow_version || 1) >= 2 ? `Codex 出题 → ${projectGeneratorActor(activeTask)} 生成 0-1 → Git 前确定${activeTask.bug_source === 'injected' ? '独立注入' : '自然'} Bug ${activeTask.bug_index || ''} → 双修复` : `Codex 出题 → ${projectGeneratorActor(activeTask)} 生成 0-1 → Codex 找到已有 Bug ${activeTask.bug_index || ''} → 双修复`) : '历史仓库题目（仅保留历史记录）'}{activeTask.trajectory_count ? ` · 当前 Bug ${activeTask.trajectory_count} 条轨迹` : ''}</dd></div>
              <div><dt>生成分工</dt><dd>{activeTask.project_prompt_author === 'codex' && ['codex_cli', 'claude_code_cli'].includes(activeTask.project_generator) ? `Codex 设计项目题目 · ${projectGeneratorLabel(activeTask)} 生成项目` : '历史任务未记录生成分工'}</dd></div>
